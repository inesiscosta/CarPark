 ...
 */
void process_command(char *command, char *latest_date, char *latest_time, char *first_date, int *first_date_set) {
    switch(command[0]) {
        case 'p':
...
int main() {
    ...
    while (1) {
        ...
        process_command(command, latest_date, latest_time, first_date, &first_date_set);
        free(command);
    }
}